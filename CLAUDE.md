...
- **A mass phase-transition write could misjudge "overridden" a full
  tick too early, permanently locking out a dozen-plus kitchen/dining
  lights at once - root-caused and fixed with a time-bounded grace
  period, 2026-08-23.** User report: after the Night→Day transition,
  "loads of the lights ended up 'overridden' when they weren't" and had
  to be manually cleared. Investigated live before touching any code -
  trace/history retention had already rolled past the actual incident
  by the time it was reported (`clear_ownership` discards the record
  entirely, and this integration's own automation traces only retain 5
  runs), so the diagnosis leaned on indirect but strong evidence: the
  current `sensor.adaptive_lighting_write_tracking` snapshot showed
  `light.extension_right_6` rescued via `matched_via: "value"` (proving
  a context mismatch really had just happened on one of the exact
  lights named), and the kitchen/extension/dining-room area had been
  cycling on/off every 5-10 minutes all morning (heavy write traffic,
  real motion activity, not a bug). The user's own follow-up - "this
  happened as we went from 'night' to 'day' which is obviously a large
  change in bulb colour and brightness" - was the piece that pinned the
  mechanism down precisely.

  **Root cause**: a phase transition writes every light in a room at
  once - often 10-20+ devices across one shared Zigbee mesh. On the
  very next tick (30-75s later, `update_interval` + `update_jitter`),
  any device whose real round-trip confirmation genuinely hadn't landed
  yet reports back with *neither* its context.id *nor* its
  brightness/colour changed from before the write - so neither
  `classify()`'s context check nor its value-rescue
  (`target_matches_values`, the PR #78 delayed-echo fix) can match, and
  it concluded `"overridden"` a full tick too early. Because
  `build_groups()` excludes anything classify() calls externally-set,
  and `write_tracker.async_record()` is only ever called for entities
  actually written (see its own docstring), that one early misjudgment
  is permanent, not transient: nothing ever re-examines an excluded
  entity except an unrelated event (the light turning off, going
  unavailable and reconnecting, or a manual force/`clear_ownership`) -
  see `grouping.py`'s `externally_set()` docstring, point 5, for the
  full mechanism. Confirmed via `AskUserQuestion` this is a recurrence
  of a gap already flagged and deliberately left unfixed on 2026-08-22
  ("Not fixed structurally this session... will keep producing new
  lockouts"), and that the user wanted the structural fix pursued now
  rather than deferred again.

  **Fix**: `override_protection.classify()` gained a time-bounded grace
  period - `PENDING_GRACE_SECONDS = 120` (confirmed with the user,
  covering a worst-case tick interval plus a full extra tick for a
  genuinely slow/congested round-trip) - extending the *same* leniency
  classify() already gives a light's very first-ever write (no
  `confirmed` yet → `"unclaimed"`, not blocking) to *any* `pending`
  claim that's still fresh, not just when there's no confirmed baseline
  at all: if neither context nor value match, but `pending` was
  recorded within the grace window, the result is `"pending"` (not
  `"overridden"`), `matched_via="grace"` - a new value alongside
  `"context"`/`"value"` so a diagnostic viewer can tell this case apart
  from an ordinary fast context match. Implemented as
  `_pending_still_fresh(pending, now, grace_seconds)`, comparing
  `pending["recorded_at"]` (already stamped on every real write by
  `write_tracking.py`'s `async_record` - nothing new needed there)
  against a `now` the caller must supply - `classify()` takes no wall
  clock reading itself, staying pure and deterministic for testing; a
  caller that omits `now` (the default) gets the original, stricter
  behaviour, not silent leniency.

  Threading `now` through required touching every real caller:
  `EntityLookup` (`grouping.py`) gained two new injected fields,
  `pending_recorded_at` and `now` (a zero-arg callable, matching the
  existing dependency-injection pattern for everything HA-touching in
  this module) - `_build_lookup()` (`__init__.py`) wires them to
  `tracker.pending_recorded_at` and `dt_util.utcnow` respectively.
  `write_tracking.py` gained a matching `pending_recorded_at()`
  accessor (mirrors `pending_target()`). `check_ownership`
  (`__init__.py`) and `sensor.py`'s status classification - the other
  two `classify()` call sites - both pass `now=dt_util.utcnow()`
  directly and build their own `pending` dict to include
  `recorded_at` (`check_ownership` previously omitted it entirely,
  since nothing needed it before this). `tests/fakes.py`'s
  `make_lookup()` gained matching optional `pending_recorded_ats`/`now`
  parameters, both defaulting to disabling the grace period entirely -
  every pre-existing `tests/test_grouping.py` call site needed zero
  changes as a result.

  **Real, not hypothetical, tension surfaced by the fix**: seven
  existing integration tests broke immediately, because they all
  simulate "someone else changes the light moments after our own
  write" in real (unfrozen) time - which is now exactly the shape the
  grace period is designed to protect. This is the accepted trade-off,
  not a bug: a genuine conflict landing within 120s of our own write
  now reads as `"pending"` for that window rather than being caught
  instantly, in exchange for a device's own normal latency no longer
  causing a permanent lockout - the same "self-heal over lockout when
  genuinely ambiguous" standing precedent this project has applied
  repeatedly to override protection. Fixed by wrapping each affected
  test in `freeze_time` and advancing past `PENDING_GRACE_SECONDS`
  before the "someone else changes it" step, rather than weakening the
  fix - `test_check_ownership_and_record_ownership_round_trip`,
  `test_clear_ownership_frees_a_light_stuck_overridden`,
  `test_override_protection_survives_a_real_write_tracking_round_trip`,
  `test_recovered_light_is_freed_while_an_unrelated_override_stays_protected`,
  `test_a_restart_style_unavailable_blip_does_not_clear_an_existing_record`
  (`test_services.py`), plus
  `test_status_overridden_when_live_context_and_value_both_mismatch`/
  `test_owner_id_is_none_when_nothing_currently_matches`
  (`test_write_tracking_sensor.py`).

  Eight new tests added: seven pure `classify()` tests in
  `test_override_protection.py` (within-grace → pending/grace;
  past-grace → overridden; the exact boundary, inclusive; omitting
  `now` disables the grace period entirely; a `pending` with no
  `recorded_at` gets no grace regardless of `now`; a grace-period
  `"pending"` still correctly blocks a *different* owner via
  `is_blocked()`), plus one true end-to-end test in
  `test_services.py`,
  `test_a_still_in_flight_write_is_retried_within_grace_but_excluded_once_stale`,
  proving the real `_build_lookup()`/service wiring (not just
  `classify()` in isolation) - a light with a settled `confirmed`
  baseline, a fresh `pending` from a genuinely different (transition)
  target, and a *third*, unrelated live context reporting neither the
  old nor the new values: retried within the grace window, excluded
  once it elapses. All mutation-verified individually - reverting the
  grace-period branch back to a bare `return "overridden", None, None`
  fails exactly the four tests that depend on it (three pure, one
  integration) and no others; swapping the boundary comparison from
  `<=` to `<` fails exactly the dedicated boundary test. Full suite:
  204/204.

  Dashboard card (`adaptive-lighting-write-tracking-card.js`) gained a
  `"grace"` entry in both `MATCHED_VIA_LABEL`
  ("recent write, not yet confirmed") and `MATCHED_VIA_TOOLTIP` -
  verified in-browser via `dashboard/preview.html` (served over HTTP,
  not `file://`) with a new synthetic `light.extension_right_3` entity
  mirroring the real incident's shape exactly (an old settled
  `confirmed` from Night phase, a one-minute-old `pending` targeting
  Day's values, a live context matching neither) - confirmed via direct
  shadow-DOM inspection that the badge renders `"Pending"`, the new
  annotation text and tooltip both render correctly, and the row sorts
  into the existing "most interesting first" order alongside the
  pre-existing `Overridden`/`Pending`/`Unavailable`/`Controlled`/`Off`
  rows. `docs/HELPERS.md`'s "Override protection" section gained a new
  paragraph describing the grace period as an extension of step 7's
  existing value-rescue exception, not a separate mechanism.
